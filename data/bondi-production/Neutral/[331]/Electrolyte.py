...
def prepare_slab(relaxed_bulk: Atoms) -> Atoms:
    ...
    """
    slab = read("/scratch/ba3g18/Lithium/Pre-opt/[331]/QE.traj", index=-2)
...
def main() -> None:
    ...
    slab_input_data["control"].update(
        {"etot_conv_thr": 1.0e-5, "forc_conv_thr": 3.0e-5}
    )
    slab_input_data["electrons"].update(
        {
            "mixing_beta": 0.1,
            "mixing_mode": "local-TF",
            "conv_thr": 1.0e-14 if EXTERNAL else 1.0e-12,  
            "diagonalization": "david",
        }
    )
